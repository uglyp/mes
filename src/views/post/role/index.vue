          ...
          <template slot-scope="scope">
            <span class="table-box-action">
              <i
                ...
                title="查看"
                @click="handleAction('view', scope.row.roleid)"
              ></i>
              <i
                class="iconfont iconziyuan mr15"
                title="编辑"
                @click="handleAction('edit', scope.row.roleid)"
              ></i>
              <i
                class="iconfont iconrenyuandangan mr15"
                title="角色菜单"
                @click="handleAction('menu', scope.row.roleid)"
              ></i>
              <i
...
export default {
  ...
  methods: {
    ...
    getTableData(params) {
      ...
      this.$api.role.select(args).then(res => {
    ...
    handleRemove() {
      ...
      this.$confirm('确定要删除角色吗?', '提示', {
        ...
        .then(() => {
          const roleid = multipleSelection.map(item => item.roleid)
          this.$api.role.delete({ roleids: roleid.toString() }).then(() => {
