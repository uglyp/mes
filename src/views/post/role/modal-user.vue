  ...
  <div class="layout">
    <div class="main">
      ...
      <section class="right-box">
        <span class="header">{{rolename}}</span>
        <div class="body">
          <div class="body-search">
  ...
  methods: {
    getAllUser() {
      this.$api.sysEmployee
        .queryAllByEmpId({ rid: this.roleid, empname: this.empname })
        .then(res => {
    ...
    getSelectUser() {
      this.$api.role.selectEmp({ rid: this.roleid }).then(res => {
    ...
    handleSubmit() {
      ...
      this.$api.role.updateRolePeo(params).then(() => {
        this.$message.success('角色人员配置成功')
        this.$router.go(-1)
      })
    },
